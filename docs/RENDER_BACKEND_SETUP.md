...
`NEXT_PUBLIC_BACKEND_API_URL` to `http://localhost:3001` (or whatever `PORT` you run
`server` on locally).
...
`NEXT_PUBLIC_BACKEND_API_URL` pointed at it, run `npm run dev` in the repo root and:
